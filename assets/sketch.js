...
// blob styling
let blobCount = 2; // total number of blobs
let radius;
let radiusFactor = 0.3; // radius of the blob (relative to canvas size)
...
let textImg;
let bgImg1, bgImg2; // images for each layer
...
/* - - Preload - - */
function preload() {
  textImg = loadImage("assets/text.png"); // text (background image)
  bgImg1 = loadImage("assets/img-1.png"); // layer 1
  bgImg2 = loadImage("assets/img-2.png"); // layer 2
}
...
function setup() {
  ...
  /* - - Images & masks - - */
  // image scaling
  calculateImageScaling(textImg);
  calculateImageScaling(bgImg1);
...
function draw() {
  ...
  if (showBG) {
    let logoHeight = textImg.height * (width / textImg.width);
  ...
  for (let i = 0; i < blobCount; i++) {
    ...
    if (i === 0) {
      ...
      reveals[i].image(
        ...
        width / 2, // x pos
        height / 2, // y pos
        bgImg1.scaledWidth, // width
        bgImg1.scaledHeight // height
      );
